...
\section{DATA ORIENTED DESIGN DUE TO NUMBA}

Data oriented design is concerned with designing code that works on simple data structures, such as arrays, in order to optimally take advantage of modern hardware features such as cache-locality, and are easy to parallelize. This contrasts with object oriented design, where although code is organized around data - the focus is on user created objects where the actual layout of the data is hidden behind abstraction, making it difficult to write code to take advantage of cache locality.

Numba's framework strongly encourages data oriented design principles, as its core focus is operations on ndarrays. This is reflected in the design of PyExaFMM's octrees as well as its API.

Octrees can either be `pointer based', or `linear'. A pointer based octree, as implemented by ExaFMM-T, uses objects to represent each node, with fields for a unique id, particle it contains, associated expansion coefficients or potentials, and pointers to their parent and sibling nodes. This makes searching for neighbors and siblings easy, as one has to just follow pointers. The linear octree, as implemented by PyExaFMM represents nodes by a unique id stored in a 1D vector, all other data such as expansion coefficients, particle data, and calculated potentials, are also stored in 1D vectors. Data is looked up by creating indices to tie a node's unique id to the associated data. This design choice is due to the constraints imposed by Numba, which makes working with arrays easy, but working with more complex user defined objects much harder. This is a good example of how Numba can affect design decisions, which may make software more complex.

Figure (\ref{fig:design}) illustrates PyExaFMM's API.
...
The P2P, P2M, P2L, M2P, L2P and near field operators are parallelized over their targets, the leaf nodes. For the L2P, M2P, and near field operators we encourage cache locality for the P2P step, and keep the data structures passed to Numba as simple as possible, by allocating 1D vectors for the source positions, target positions and the source charges (or expansion coefficients - depending on the operator), such that all the data required to apply an operator to single target node is next to each other in memory. We store a vector of `index pointers', that allow us to bookend data for corresponding to different targets. We pass then this data to a P2P implementation parallelized over each leaf node.

For the L2P and P2M operators, each target node is only using it's own associated data, whereas for the near field operator the source nodes are from its U list, the size of which is apriori unknown. To bypass this, we allocate enough memory to store up to 60 nodes worth of data for each target, corresponding to the maximum size of the U list. This strategy is too expensive in terms of memory to repeat for the M2L and M2P operators, as their interaction lists can potentially be much larger, at 189 and 146 nodes respectively. For these operators, we simply perform a parallel loop over the target nodes at given each level for the M2L, and over the leaf nodes for the M2P, looking up the relevant data from the linear tree. We also don't implement the cache-optimal strategy for the P2L operator, the interaction list is at most 19 nodes, and we found in practice that it did not benefit from it significantly.
