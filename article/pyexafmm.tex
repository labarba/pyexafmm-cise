...
\begin{abstract}
    Numba is a game changing compiler for high performance computing with Python. The machine code it produces runs outside of the single-threaded Python interpreter, and can therefore fully utilize the resources of modern CPUs. This means support for parallel multithreading and auto vectorization if available, just like in a compiled language such as C++ or Fortran. In this paper we present PyExaFMM, a fully multithreaded Numba implementation of the Fast Multipole Method, an algorithm which relies on a non-linear data structure and contains a significant amount of data organization. We find that designing highly performant Numba code can be as challenging as writing in a compiled language, and that there is a significant runtime cost from the interaction between Numba compiled code and the Python interpreter.
\end{abstract}
...
We begin with an overview of Numba's design and its major pitfalls when. After introducing the data structures and computations involved in the FMM, we provide an overview of how we implemented our software's data structures, algorithms and application programming interface [API] to optimally use Numba.
...
Multithreading in this way means that we call the P2P, P2M, M2P, L2P and near field operators just once during the algorithm, the M2L is called $d-2$ times, and the M2M and L2L are called $d$ times where $d$ is the depth of the octree. This minimizes the number of times we can pass control between Python and Numba for this algorithm while being able to keep the operators separate in API of PyExaFMM for unit testing.
...
\begin{IEEEbiography}{Srinath Kailasa}{\,}is a PhD student in Mathematics at University College London. His background is Physics and Computer Science, with an MPhys from Durham and an MSc from UCL. His research interests are centered around applying mathematics and computation to problems in science and engineering. Contact him at srinath.kailasa.18@ucl.ac.uk.
\end{IEEEbiography}
