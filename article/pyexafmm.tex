...
\begin{abstract}
Numba is a compiler for numerical CPython code that uses Numpy's ndarray data structure, and is able target a range of hardware from modern multicore CPUs to GPUs. As the machine code produced by Numba runs outside of the Python interpreter, it is possible to produce multithreaded, or autovectorized, CPU code that wouldn't ordinarily be possible in pure Python. In this paper we illustrate how Numba can be used to develop a multithreaded Python application with a complex non-linear data structure, and significant data organization that would ordinarily take place within the Python interpreter via PyExaFMM, an implementation of the Fast Multipole Method. We report that our software remains up to 30 times slower than ExaFMM-T, the leading C++ implementation of the same algorithm, for computing results with the same accuracy due to the unavoidable interaction between the interpreter and the Numba compiled functions. However, considerably faster [INSERT BENCH HERE] than a vanilla Python implementation. Working in Python and Numba is therefore a good choice for projects where the ability to rapidly iterate on prototypes is a greater priority than pure performance, or with limited amounts of data organization and linear data structures.
\end{abstract}

\maketitle
\chapterinitial{Chapter Initial}

- CPython is designed for safety not speed. 
- Introduce concept of GIL, and that GIL is only an issue in scientific computing if significant amounts of time is spent within the interpreter (numpy and scipy and how they get past this)
- More complex algorithms cannot pass of majority of compute time outside of GIL without say passing to a custom built C extension, enter Numba, offers a way of doing this without writing in another language
- 
