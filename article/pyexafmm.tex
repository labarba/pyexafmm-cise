...
This is an artificial example and is designed to illustrate how small changes to writing style can impact the performance of an algorithm. The main point being that a user's ignorance of how Numba is compiling their code can lead to a loss of performance.
...
Indeed repeating the experiment with $A \in \mathbb{R}^{1000 \times 1000}$ and $v \in \mathbb{R}^{1000}$, we obtain $27.1 \pm 0.2 ms$, $27.1 \pm 0.1 ms$ and $2.84 \pm 0.01 \mu s$, reflecting that inlining is still the dominant factor in performance difference and is even more prominent than with the smaller dataset.

With $A \in \mathbb{R}^{1 \times 1}$ and $v \in \mathbb{R}^1$, we obtain $1.74 \pm 0.01 \mu s$, $2.94 \pm 0.01 \mu s$ and $2.61 \pm 0.01 \mu s$ for the three implementations respectively, reflecting that the instantiation of the result dictionary from within a Numba function is now a significant part of total runtime.

Nested functions have the tendency to grow long in performant Numba code, in order to minimize the number of interactions between Numba and Python. However this makes them more difficult to unit test. Indeed, performant Numba code can look decidedly un-Pythonic, with Numba encouraging few user created objects, performance critical sections written in terms of loops over simple array based data structures, and potentially long nested functions. 

Though Numba is advertised as an easy way of injecting performance into your program via a simple decorator it has its own learning curve. Achieving performance requires a programmer to be familiar with the internals of its implementation, and potentially have to significantly alter the design of their algorithms and data structures.
