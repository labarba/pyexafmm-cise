...
Numba compilation makes many of PyExaFMM's functions competitive with ExaFMM-T's C++ implementation. Our implementation of (\ref{eq:laplace_kernel}) evaluates interactions between 100,000 sources and targets in $4.7 \pm 0.3$ s compared to $2.00 \pm 0.01$ s with ExaFMM-T. For a problem with 1,000,000 randomly distributed particles, and a constraint of at most 150 particles per leaf box, tree building and balancing takes $2.75 \pm 0.04$ s compared to $0.64 \pm 0.01$ s in ExaFMM-T,

Numba has a large impact on functions that use simple instructions, for example tree operations which use only bit shifts or arithmetic instructions, experience a large speed up. Finding the neighbors of a given box takes $1.041 \pm 0.001$ $\mu$s compared to $63 \pm 1$ $\mu$s without Numba, finding a box's parent takes $171 \pm 1$ ns compared to $573 \pm 3$ ns and finding a box's children takes \dots

Operator pre-computation times depend on the expansion order of the multipole and local expansions, $p$, as well as the depth of the tree. However it is fast enough not to significantly impact on workflow. For a problem with $p=10$ local and multipole expansions, it takes ...

Table (\ref{tab:performance}) illustrates the accuracy, peak memory consumption and runtime of PyExaFMM with respect to ExaFMM-T. Experiments are run for 1,000,000 source and target particles distributed in two geometries: distributed randomly in a cubic unit box, and distributed randomly on the surface of a sphere with unit radius, and a constraint of at most 150 particles per leaf box. We observe similar errors and memory usage between the two softwares, runtime increases as a function of expansion order $p$ and compression parameter $k$.

From figure (\ref{fig:operator_runtimes}) we see that for similar expansion orders the operator implementations for both softwares take similar fractions of total runtime. This implies that PyExaFMM shares a similar computational complexity with ExaFMM-T, and that there must be a large constant added to runtime, due to Python related overhead. PyExaFMM is run in two regimes: low $k$ and high $k$, to test the impact of compression parameter on the composition of fractional runtimes. We observe that it leads to an increase in the fraction dedicated to the M2L operator from ... to ... .

% Smaller figure
\begin{figure}
	\centerline{\includegraphics {figures/operator_runtimes.pdf}}
	\caption{Fractional runtimes broken down by operator for (a) ExaFMM-T $p=5$ (b) ExaFMM-T $p=10$ (c) PyExaFMM $p=5$ $k=50$ (d) PyExaFMM $p=10$ $k=50$ (e) PyExaFMM $p=10$ $k=150$}
