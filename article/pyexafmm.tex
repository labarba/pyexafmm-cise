...
\begin{abstract}
	Numba is a `just in time' [JIT] compiler for CPython that compiles Python code operating on Numpy's ndarray data structure into architecture optimized machine code which can seamlessly be called from a Python application. Usage is pain-free, users simply mark a function for optimization with a decorator, and Numba will run the compiler at runtime if the marked function is called. Numba's generic design allows it to compile to different hardware targets, including multicore CPUs and GPUs. Thus Numba offers a solution to the lack of native multithreading in CPython, and the prospect of writing Python code that can be optimized for modern CPUs painlessly. In this paper we illustrate how Numba must be used in order to develop multithreaded software for complex algorithms, with significant data organization and a complex data structures through our software PyExaFMM\footnote{https://www.github.com/exafmm/pyexafmm} , an implementation of the Fast Multipole Method for a single node. We report benchmarks with respect a vanilla Python implementation, and offer a sanity check on the performance in terms of speed and accuracy by comparing with ExaFMM-T, the leading single node C++ implementation of the same algorithm. We report that our software remains between 10 and 30 times slower than the C++ implementation for the same accuracy due to the unavoidable interaction between the interpreter and the Numba compiled functions. However, considerably faster ......... than a pure Python implementation. By writing in Python we retain the benefits of a simple simple language, trivial cross platform builds, and a large open-source community. Therefore Numba remains a good choice for projects where development iteration is a larger priority than performance.
\end{abstract}
