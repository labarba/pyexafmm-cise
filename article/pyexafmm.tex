...
Numba is a compiler for Python that targets a subset of the language that manipulates the $n$-dimensional array data structure from Numpy, the ndarray. Unlike a Python list, where each element is linked to the previous element by a pointer, and elements can be of any type, Numpy arrays are homogeneously typed and stored contiguously in memory. This means that adjacent array elements are stored in adjacent memory locations, which results in two benefits for scientific computing. Firstly, as modern CPUs tend to cache memory at adjacent registers when asked to load data from a given address, this memory layout is useful in algorithms which iterate over arrays as the CPU's cache is likely to contain the next element being operated on. This is known as \textit{locality of reference}. Secondly, the contiguous nature of the array means Numpy is able to calculate the memory addresses of sub-arrays using its knowledge of the pointer to the start of the array, the data type, and the number of elements in the slice. This is in contrast to Python lists, where subsequent element are found by following trails of pointers, known as \textit{indirection}.

Numba was built to further optimize operations using ndarrays, specifically

- loop fusion, indirection.


Numba is built using LLVM [FURTHER READING ON LLVM], a tool for building custom compilers, that provides an API to generate machine code for different hardware architectures such as CPUs and GPUs. LLVM is also able to analyze the code it's compiling for hardware level optimizations such as auto vectorization. So Numba attempts to optimize Python code for your hardware

From a user perspective, Numba doesn't interfere with workflow - functions are marked for compilation with a special decorator (see listings) and Numba's runtime compiles functions on the fly, when they are called from the Python interpreter.

instead  functions are marked for Numba compilation with a decorator (see listings (\ref{code:loop_fusion}) and (\ref{code:parallel_multithreading})), and when called at runtime compiled on the fly. The execution path of what happens when a Numbafied function is called from the Python interpreter is illustrated in figure (\ref{fig:numba}).


Despite being a compiler, Numba doesn't replace the Python interpreter. As seen from figure (\ref{fig:numba}), Numba interacts with the Python interpter dynamically and the control of program execution is passed back and forth between the Numba runtime and the intereter. There is an inherent cost to this interaction which is due to having to `box' and unbox data types between types compatible with Python and the native compiled numba function. [INSERT BENCHMARK EXPERIMENT HERE FOR BOXING AND UNBOXING COST]
