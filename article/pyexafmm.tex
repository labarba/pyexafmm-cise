...
Numba is a compiler built to target a subset of Python code that uses ndarrays, and is built with LLVM, a framework for building custom compilers. LLVM provides an API for generating machine code for different hardware architectures such as CPUs and GPUs and is also able to analyze the code for hardware level optimizations such as auto vectorization, automatically applying them if they are available on the target hardware \cite{Lattner2004}. LLVM is able to generate multithreaded code, which isn't available in ordinary Python due to the GIL. Furthermore Numba is able to use the metadata provided by ndarrays describing their dimensionality, type and layout to generate code that takes advantage of the hierarchical caches available in modern CPUs \cite{Lam2015}. Altogether, this allows code generated by Numba to run significantly faster than ordinary Python code, and in certain circumstances be competitive with code generated from compiled languages such as C++ or Fortran. 

From a programmer perspective using Numba, at least naively, doesn't involve a significant rewrite. Python functions are simply marked for compilation with a special decorator, see listings (\ref{code:loop_fusion}), (\ref{code:nested_function}) and (\ref{code:parallel_multithreading}) for example syntax. This is why the promise of Numba is so appealing. Generating high-performance code while writing in Python, and letting Numba worry about how to perform optimizations, would allow for much faster prototyping than possible with a compiled language. For Computational Scientists, this means more time thinking about science than the details of a software implementation, and writing high-performance software without the headaches of compiled languages.
...
An example of Numba's framework-like behavior arises when implementing algorithms that share data, and have multiple logical steps as in listing (\ref{code:nested_function}). This listing shows three implementations of the same logic, a function that generates a random matrix $A \in \mathbb{R}^{100 \times 100}$ and multiplies it with itself, and also writes some input vector $v \in \mathbb{R}^{100}$ to a Numba dictionary. Data of this size is chosen to reflect the typical amount of computation in a task parallelized by PyExaFMM. 

The implementations \lstinline{algorithm1} and \lstinline{algorithm2} aren't distinguished by the Numba compiler, and both pay a cost to call subroutines defined outside of their function body. However, \lstinline{algorithm1} pays an additional (un)boxing cost in order manipulate a globally defined Numba compatible dictionary, in comparison to a locally defined one in \lstinline{algorithm2}. This is reflected in their runtimes, where \lstinline{algorithm1} runs in $308 \pm 1 \mu s$ and \lstinline{algorithm2} runs in $306 \pm 1 \mu s$\footnote{Statistics calculated over seven trials}, respectively\footnote{All experiments are taken on an AMD Ryzen Threadripper 3970X 32-Core processor running Python 3.8.5 and Numba 0.53.0}.

The \textit{nested} function in \lstinline{algorithm3} differs from the other two implementation, by defining its sub-routines within its function body, rather than calling externally defined functions. This leads to a faster runtime, and is an example of an \textit{inlining} optimization being taken by LLVM. Inlining allows \lstinline{algorithm3} to run in just $2.64 \pm 0.07 \mu s$.

This is an artificial example and is designed to illustrate how small changes to writing style can impact the performance of an algorithm. The main point being that the users ignorance of how Numba is interpreting their code can lead to a loss of performance.

We emphasize that the speedup obtained from inlining is dependent on the size of the data being operated on as well as the program logic. Other factors such as memory latency for large data, or the passing of execution control between Python and Numba, with small data are may become more significant. 

Indeed repeating the experiment with $A \in \mathbb{R}^{1000 \times 1000}$ and $v \in \mathbb{R}^{1000}$, we obtain $27.1 \pm 0.2 ms$, $27.1 \pm 0.1 ms$ and $2.84 \pm 0.01 \mu s$, reflecting that inlining is still the dominant factor in performance difference it's even prominent than with the smaller dataset.

With $A \in \mathbb{R}^{1 \times 1}$ and $v \in \mathbb{R}^1$, we obtain $1.74 \pm 0.01 \mu s$, $2.94 \pm 0.01 \mu s$ and $2.61 \pm 0.01 \mu s$ for the three implementations respectively, reflecting that the instantiation of the result dictionary is now a significant part of total runtime.

Nested functions have the tendency to grow long in performant Numba code, in order to minimize the number of interactions between Numba and Python. However this makes them more difficult to unit test. Indeed, performant Numba code can look decidedly un-Pythonic. Numba encourages few user created objects, performance critical sections written in terms of loops over simple array based data structures - reminiscent of C, and potentially long nested functions. 

Though Numba is advertised as an easy way of injecting performance into your program via a simple decorator it has its own (occasionally unwritten) learning curve as a user attempts to write their code to be optimized with LLVM as well as possible. Achieving performance requires a programmer to be familiar with the internals of its implementation, and potentially have to radically change the design of their algorithms and data structures to avoid back and forth between the Numba runtime and the Python interpreter.
...
The speedup achieved with this strategy, in comparison to a naive parallel iteration over the L2P's targets, increases with the number of calculations in each thread and hence the expansion order $p$. In an experiment with $n_{crit} = 150$ and $p=10$, our strategy is 13 \% faster.
