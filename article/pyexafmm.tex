...
Here we review the computational structure of each operator, to see where it's possible to insert parallelism into an KIFMM implementation.

The inputs to the P2P operator are vectors for the source and target positions, and the source charges, with the expected output being a vector of potentials. It's an embarrassingly parallel problem over each target particle, and can be used as a measure of raw performance of the Numba compiled code.

The near field, L2P, M2P, P2L and P2M operators act over all leaf nodes independently, and all rely on the P2P operator with different inputs. The M2L operator acts independently on all nodes at a given level during the top-down traversal, for nodes between level two and the leaf level of the octree.

The near field operator applied to a target node applies the P2P operator between the charges contained within the target, and the source particles for nodes in its U list. This results in $O(60 \cdot n_{crit}^2)$ calculations. The M2P operator applied to a target node, applies the P2P operator between multipole expansion coefficients of source nodes in the target's W list the charges the target contains. Similarly, the L2P operator applied to a target node, applies the P2P operator between it's own local expansion coefficients and the charges it contains. This results in $O(148 \cdot n_e \cdot n_{crit})$ and $O(n_e \cdot n_{crit})$ calculations respectively.

The P2L, P2M and M2L involve creating local and multipole expansions, and rely on a matrix vector product related to the number of source nodes being compressed, which for the P2L and M2L are defined by the size of the target node's interaction lists. These matrix vector products have complexities of the form $O(k \cdot n_e^2)$ where $k = |X| = 19$ for the P2L operator, $k = |V| = 189$ for the M2L operator, and $k = 1$ for the P2M operator. Additionally, the P2L and P2M operators have to calculate `check potentials' (see sec. 3 \cite{Ying2004}) which require $O(19 \cdot n_{crit} \cdot n_c)$ and $O(n_{crit} \cdot n_c)$ calculations respectively.

The M2M operator is applied to each node during the bottom-up traversal, and the L2L operator is applied to each node during the top-down traversal. They both involve translating expansions between nodes their eight children, and rely on a matrix vector product of $O(n_e^2)$.

The FMM's operators expose natural parallelism. The M2L, M2P, P2L and near field operators operate independently over their interaction lists. Furthermore, the near field, L2P, M2P, P2L and P2M operators also operate independently over all the leaf nodes, and the M2L operates independently over the nodes at a given level. Finally, The M2M and L2L operators operate independently over sibling nodes. We note that some parallelizations incur a cost in the form of writing to shared memory locations, an operator parallelized over its interaction list will write to a shared vector for the target node's expansions or potentials. This also applies to the M2M and L2L operators, when parallelized over sibling nodes.
