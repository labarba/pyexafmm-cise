...
The implementations \lstinline{algorithm1} and \lstinline{algorithm2} aren't distinguished by the Numba compiler, and both pay a cost to call subroutines defined outside of their function body. However, \lstinline{algorithm1} pays a small additional (un)boxing cost in order to manipulate a globally defined Numba compatible dictionary, in comparison to a locally defined one in \lstinline{algorithm2}. The \textit{nested} function in \lstinline{algorithm3} differs from the other two implementation, by defining its sub-routines within its function body, rather than calling externally defined functions. This is an example of an inlining optimization, which LLVM is capable of picking up at compile time. Inlining optimizations are a beneficial technique as they reduces function call overhead and encourages code locality, ultimately enhancing execution speed and overall program performance. However, here Numba optimizes out the matrix vector product as it's not being stored in a result being used. This is reflected in the runtimes of all three implementations, shown in table (\ref{tab:boxing_inlining}) for different problem sizes \footnote{All experiments in this work were taken on an AMD Ryzen Threadripper 3970X 32-Core processor running Python 3.8.5 and Numba 0.53.0}. This example is designed to illustrate how arbitrary changes to writing style can impact the behaviour of Numba code. Other factors such as memory latency for large data or the passing of execution control between Python and Numba with small data may also be significant. For example, with $A \in \mathbb{R}^{1 \times 1}$ and $v \in \mathbb{R}^1$, the instantiation of the result dictionary from within a Numba function is a significant part of total runtime, as shown by the difference in performance between \lstinline{algorithm1} and \lstinline{algorithm2}.
...
Therefore one has to take care when inlining in this way to ensure that their intermediate results are not dropped. Indeed, a performant Numba code in which the number of interactions between Numba and Python are minimized, and inlined code is maximized, could look decidedly un-Pythonic. Furthermore, unit testing such code is difficult to its lack of modularity. Numba can be seen to encourage few user-created objects, performance-critical sections written in terms of loops over simple array based data structures, and potentially long nested functions in order to achieve performance.
...
To achieve optimal multithreaded performance with Numba, careful consideration of the algorithm, Numba's backend implementation, and a data-oriented design are required. The pitfalls illustrated above demonstrate the need for care when adapting code to achieve performance with Numba. While Numba is marketed as a simple way to enhance performance of Python code with a decorator, implementing complex algorithms with Numba requires significant software development expertise. This level of expertise may exceed the capabilities of Numba's target audience.
