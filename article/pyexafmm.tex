...
We emphasize that the speedup obtained from inlining is dependent on the size of the data being operated on as well as the program logic. Other factors such as memory latency for large data, or the passing of execution control between Python and Numba, with small data are may become more significant. Indeed repeating the experiment with $A \in \mathbb{R}^{1000 \times 1000}$ and $v \in \mathbb{R}^{1000}$, we obtain $27.1 \pm 0.2 ms$, $27.1 \pm 0.1 ms$ and $2.84 \pm 0.01 \mu s$, reflecting that inlining is still the dominant factor in performance difference and is even more prominent than with the smaller dataset. With $A \in \mathbb{R}^{1 \times 1}$ and $v \in \mathbb{R}^1$, we obtain $1.74 \pm 0.01 \mu s$, $2.94 \pm 0.01 \mu s$ and $2.61 \pm 0.01 \mu s$ for the three implementations respectively, reflecting that the instantiation of the result dictionary from within a Numba function is now a significant part of total runtime.
