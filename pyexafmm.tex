...
\begin{abstract}
We present PyExaFMM, a kernel-independent particle fast multipole method (FMM) implementation, built on the success of the ExaFMM project, to answer the question: can we develop a highly-performant scientific code, without resorting to a lower level language, that is competitive with the state of the art C++ implementation? The FMM represents a good case study for understanding the utility of Python's high-performance ecosystem. Performant FMM implementations have to take special care to handle the complex hierarchical adaptive-octree datastructure on which it is dependent. As many of Python's numerical libraries are built to accelerate computations over simple arrays, making PyExaFMM performant is not as trivial as the availability of `drop-in' tools may make it seem.
\end{abstract}
